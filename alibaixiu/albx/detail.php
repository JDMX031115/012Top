...
<script type="text/template" id='detailtp'>
  <div class="breadcrumb">
    <dl>
      <dt>当前位置：</dt>
      <dd><a href="list.php?id={{$data.category_id}}">{{$data.name}}</a></dd>
      <dd>{{$data.title}}</dd>
    </dl>
  </div>
  <h2 class="title">
    <a href="detail.php?id={{$data.id}}">{{$data.title}}</a>
  </h2>
  <div class="meta">
    <span>{{$data.nickname}} 发布于 {{$data.created}}</span>
    <span>分类: <a href="list.php?id={{$data.category_id}}">{{$data.name}}</a></span>
    <span>阅读: ({{$data.views}})</span>
    <span>喜欢: ({{$data.likes}})</span>
  </div>
  <div class="content-detail">{{$data.content}}</div>
</script>
<script src="/static/assets/js/_detail.js"></script>
