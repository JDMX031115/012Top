$(function () {
  ...
  function checkData() {
    ...
    var slug = $("#slug").val();
    if (name.trim() == '') {
      var msg = $('.alert').clone(true);
      msg.removeClass('hidden').children('.msg').text('分类名称不能为空');
      $('.container-fluid').append(msg);
      return;
    }
    if (slug.trim() == '') {
      var msg = $('.alert').clone(true);
      msg.removeClass('hidden').children('.msg').text('分类别名不能为空');
      $('.container-fluid').append(msg);
      return;
    }
  }
  function showMsg(msg,fn) {
    $(".modal-body").text(msg);
    $("#msgModal").modal('show').on('hidden.bs.modal', function () {
      fn && fn();
    });
  }
  ...
  $("#add").on('click', function () {
    ...
    $.ajax({
      url: '/admin/api/_addCategory.php',
      type: 'POST',
      data: data,
      dataType: 'json',
      success: function (res) {
  ...
  $('tbody').on('click', '.edit', function () {
    ...
    var slug = tr.children().eq(2).text();
    var classname = tr.children().eq(3).children('span').attr('class');
  ...
  $("#save").on('click', function () {
    ...
    $.ajax({
      url: '/admin/api/_updateCategoryById.php',
      type: 'POST',
      data: fd,
      contentType: false,
      processData: false,
  ...
  $('tbody').on('click', '.del', function () {
    ...
    $.ajax({
      url: '/admin/api/_delCategoryById.php',
      type: 'POST',
      data: {
        id: id
      },
  ...
  $("#mult-del").on('click', function () {
    ...
    checked.each(function (i, e) {
      var id = $(e).parent().parent().attr('categoryid');
      ids.push(id);
    });
